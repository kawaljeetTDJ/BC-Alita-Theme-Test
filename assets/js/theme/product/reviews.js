...
export default class {
    constructor($reviewForm) {
        ...
        this.$collapsible = $('.productView-reviewTabLink');
     ...
     */
    initLinkBind() {
        const $content = $('#tab-reviews-mobile', this.$reviewsContent);
    ...
    collapseReviews() {
        ...
        if (window.location.hash && window.location.hash.indexOf('#tab-reviews') === 0) {
            if($(window).width() > 1024){
                this.$collapsible.trigger('click');
            } else{
                $('.toggleLink', this.$reviewsContent).trigger('click');
            }
        }
