...
export default function (secureBaseUrl, cartId, context) {
    ...
    $(document).on('click', event => {
        if (($(event.target).closest('[data-cart-preview]').length === 0) && ($(event.target).closest('#halo-cart-sidebar').length === 0) && ($(event.target).closest('#modal').length === 0) && ($(event.target).closest('[data-cart-edit-item-remove]').length === 0) && ($(event.target).closest('.card-footer').length === 0)){
